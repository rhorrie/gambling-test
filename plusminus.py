...
for i in range(0, len(mlb_team_array)):
	con.execute('UPDATE mlb_gambling SET plusminus = ? WHERE name = ?', ((mlb_plusminus_array[i]), mlb_team_array[i]))
...
for i in range(0, len(nfl_team_array)):
	con.execute('UPDATE nfl_gambling SET plusminus = ? WHERE name = ?', ((nfl_plusminus_array[i]), nfl_team_array[i]))
